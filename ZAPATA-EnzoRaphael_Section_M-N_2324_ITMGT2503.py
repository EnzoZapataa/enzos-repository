...
jsondata_upper = str(jsondata_upper).upper()
jsondata_removed = jsondata_upper.replace('<P>', '').replace('</P>', '').replace('<B>', '').replace('</B>','').replace('<I>','').replace('</I>','')
jsondata_removed = jsondata_removed.replace('DICT_VALUES','')
unnecessary_punctuation = r"&$@[].,'#()-\"!?’_;:/"
char_remov = list(unnecessary_punctuation)

for char in char_remov:
    jsondata_removed = jsondata_removed.replace(char, " ")

print(jsondata_removed)
...
for word in jsondata_removed.split():
        
        if len(word) >= 3 and word in wordcount_dictionary:
            wordcount_dictionary[word] += 1
        
        elif len(word) <= 2 and word in wordcount_dictionary:
            del(wordcount_dictionary[word])
...
# optimal_Z = 10*optimal_F + 12*optimal_R + 16*optimal_C #Maximize
# lumber_constraint = 500*optimal_F + 750*optimal_R + 300*optimal_C
# food_constraint = 50*optimal_F + 45 * optimal_R + 75 * optimal_R


for i in range(0,optimal_F+1):
    for j in range(0, optimal_R+1):
        for k in range(optimal_C+1):
