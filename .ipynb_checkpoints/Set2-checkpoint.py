...
def shift_letter(letter, shift):
    ...
    *Note: the single underscore `_` is used to acknowledge the presence
    ...
    if letter == ' ':
        ...
        current_letter = 'ABCDEFGHIJKLMNOPQRSTUVWXYZ'.index(letter)
        new_letter = (current_letter + shift) % 26
        return 'ABCDEFGHIJKLMNOPQRSTUVWXYZ'[new_letter]
...
def shift_by_letter(letter, letter_shift):
    ...
    shifted_index = (original_index + shift_value) % 26
...
def vigenere_cipher(message, key):
    ...
    for i in range(message_length):
        ...
        if char == ' ':
            ...
            shifted_index = (alphabet.index(char) + key_index) % 26
...
def scytale_cipher(message, shift):
    ...
    for i in range(rows):
        for j in range(shift):
            encoded_message += message[j * rows + i]
...
def scytale_decipher(message, shift):
    ...
    decoded_message = [''] * (columns * rows)
    
    for i in range(columns):
        for j in range(rows):
            index = i + j * columns
            decoded_message[index] = message[i * rows + j]
