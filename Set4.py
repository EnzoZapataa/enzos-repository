...
def bin_to_dec(binary_string):
    ...
    number_of_element = 0
    max_number_of_element = len(binary_string)
    initial_num = 0
    
    while number_of_element < max_number_of_element:
        for i in range(max_number_of_element):
            value_of_each_element = int(binary_string[i]) * (2 ** (max_number_of_element - 1 - i))
            initial_num += value_of_each_element
            number_of_element += 1
...
def dec_to_bin(number):
    ...
    while number > 0:
        final_ans.append(str(number % 2))
        number = number // 2
        
    return ''.join(final_ans[::-1])
...
def telephone_cipher(message):
    ...
    for element in message:
        current_digits = encoder_dict[element]
        if previous_digit and current_digits[0] == previous_digit:
            final_answer.append('_')
        final_answer.append(current_digits)
        previous_digit = current_digits[0]
...
def telephone_decipher(telephone_string):
    ...
    while i < len(telephone_string):
        if telephone_string[i] == '_':
            ...
            start = i
            while i < len(telephone_string) and telephone_string[i] == telephone_string[start]:
                i += 1
            length = i - start
            key = telephone_string[start:start + length]
